...
class _RegistroAlimentosPageState extends State<RegistroAlimentosPage> {
  ...
  Future<void> _initializeData() async {
    ...
    if (id != null) {
      final ingestas = await ingestaService.getIngestasByUsuarioId(id);
      ...
      for (final ingesta in ingestas) {
        ...
        if (!addedAlimentoIds.contains(ingesta.alimento_id)) {
          final alimento = alimentosMap[ingesta.alimento_id];
          if (alimento != null) {
            historialTemp.add(alimento);
            addedAlimentoIds.add(ingesta.alimento_id);
          }
        }
      }
  ...
  void _guardarIngestas() async {
    ...
    for (final alimento in _alimentosSeleccionados) {
      ...
      await ingestaService.addIngesta(ingesta);
    }
